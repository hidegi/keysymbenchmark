...
static const unsigned short ucstable[] =
{
    ///XK_LATIN2 (57) + 38
    // 0x01a1 - 0x01ff
    ...
    ///XK_DEAD (42) + 25
    // 0xfe50 - 0xfe92
    '`'   , 0x00b4, '^'   , '~'   , 0x00af, 0x02d8, 0x02d9, 0x00a8,
    0x02da, 0x02dd, 0x02c7, 0x00b8, 0x02db, 0x037a, 0x309b, 0x309c,
    0x0000, 0x0000, 0x0000, '/'   , 0x02bc, 0x02bd, 0x02f5, 0x02f3,
    0x02cd, 0xa788, 0x02f7, 0x0000, 0x0000, 0x0000, ','   , 0x00a4,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    'a'   , 'A'   , 'e'   , 'E'   ,  'i'  ,  'I'  , 'o'   , 'O'   ,
    'u'   , 'U'   , 0x0259, 0x018f, 0x00b5, 0x0000, 0x0000, 0x0000,
    '_'   , 0x02c8, 0x02cc,
    ...
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0037, 0x0034, 0x0038,
    0x0036, 0x0032, 0x0039, 0x0033, 0x0031, 0x0035, 0x0030, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, '*'   , '+'   , ','   , '-'   , '.'   , '/'   ,
...
static int getIndex(unsigned int keysym)
{
    ...
    {
        {0x01a1, 0x01ff, 0x01a1},
        {0x03a2, 0x03fe, 0x0343},
        {0x047e, 0x04df, 0x03c2},
        {0x05ac, 0x05f2, 0x048e},
        {0x06a1, 0x06ff, 0x053e},
        {0x07a1, 0x07f9, 0x05dd},
        {0x08a1, 0x08fe, 0x0684},
        {0x09e0, 0x09f8, 0x0765},
        {0x0aa1, 0x0afe, 0x080d},
        {0x0ba3, 0x0bfc, 0x08b1},
        {0x0cdf, 0x0cfa, 0x0993},
        {0x0da1, 0x0df9, 0x0a39},
        {0x0ea1, 0x0eff, 0x0ae0},
        {0x13a4, 0x13be, 0x0f84},
        {0xfe50, 0xfe92, 0xfa15},
        {0xff80, 0xffbd, 0xfb02},
    };
    int min = 0;
    int max = 15;
    int mid;
    ...
    {
        mid = (min + max) / 2;
        if (rangetuples[mid].end < keysym)
            min = mid + 1;
        else if (rangetuples[mid].start > keysym)
            max = mid - 1;
        else
...
uint32_t keysymLinear(unsigned int keysym)
{
    ...
    return ucstable[getIndex(keysym)];
}

int main()
{
    static unsigned int randomKeysyms[] =
    {
        0x01a3,
        0x0dc3,
        0x0db9,
    ...
    printf("BENCHMARKS with random keysyms:\n");
    ...
    for(i = 0; i < 15; i++)
    {
        unsigned int codepoint = keysymMap(randomKeysyms[i]);
        printf("sym 0x%04x = uni 0x%04x\n", randomKeysyms[i], codepoint);
    }

    t = clock() - t;
    double time_taken = ((double)t)/CLOCKS_PER_SEC;
    printf("duration for mapped keysyms: %f seconds\n", time_taken);
    t = clock();
    for(i = 0; i < 15; i++)
    {
        unsigned int codepoint = keysymLinear(randomKeysyms[i]);
        printf("sym 0x%04x = uni 0x%04x\n", randomKeysyms[i], codepoint);
    }

    t = clock() - t;
    time_taken = ((double)t)/CLOCKS_PER_SEC;
    printf("duration for indexed keysyms: %f seconds\n", time_taken);
